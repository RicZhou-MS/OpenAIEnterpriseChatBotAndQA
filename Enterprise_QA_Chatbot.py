...
For demo purpose, the chatbot summarizes based on the vector generated by LangChain_Ingest_All.py by scanning the docs in subfolder ..\Doc_Store.
'''
import faiss
...
with open("./Doc_Store/vectorDB.pkl", "rb") as f:
    vectorstore = pickle.load(f, encoding="utf-8")
...
lc_chatbot.max_tokens_limit = GlobalContext.TOTAL_TOKENS_LIMIT_OF_ALL_DOCS_FOR_CHAIN
...
if GlobalContext.SHOW_SINGLE_TURN_QA:  # disabled by default
    ...
    # lc_qa_chain1 = VectorDBQAWithSourcesChain.from_chain_type(lc_qa_chain_llm, chain_type="refine", vectorstore=vectorstore, k=3)  # stuff , map_reduce, refine, map_rerank
...
with gr.Blocks() as demo:
    ...
    gr.HTML('<hr size="18" width="50%" color="red">')
    ...
    ** History and Settings **
    ...
    msg.submit(user, [msg, chatbot], chatbot, queue=False).then(bot, chatbot, [msg, chatbot]).then(readOuput, None, record_button)
    ...
    QA_question.submit(QA_get_msg, QA_question, QA_Panel, queue=False).then(QA_set_panel, QA_Panel, [QA_question, QA_Panel])
